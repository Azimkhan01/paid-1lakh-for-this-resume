...
function Resume({ isLink, isPrint, isHardCopy }) {
  ...
  const handlePrint = () => {
    ...
    alert("when you redirect to print goto advance setting and turn off header and footer ")
    window.print();
  };
  ...
  return (
    <>
      ...
      <button
        id="hc"
        className={`border text-blue-400 border-blue-400 m-2 hover:bg-blue-400 hover:text-white/90 transition-all ease-in-out duration-200 rounded p-2 ${isHardCopy ? "hidden" : ""}`}
      ...
      <button
        id="ip"
        className={`border text-blue-400 border-blue-400 m-2 hover:bg-blue-400 hover:text-white/90 transition-all ease-in-out duration-200 rounded p-2 ${isPrint ? "hidden" : ""}`}
        onClick={handlePrint}
      >
        Print
      </button>
      ...
      <div ref={mainId} className="max-w-4xl mx-auto p-12 text-gray-900 font-sans space-y-6 bg-white">
        ...
        <header className="text-center space-y-1">
          ...
          <p>
            +91 {headerOne?.number} |{" "}
            <a href={`mailto:${headerOne?.email}`} className="text-blue-600 underline decoration-white">
              {headerOne?.email}
            </a>{" "}
            | {headerOne?.address}
          </p>
          {social && !hardCopy && (
            <p>
              {social.linkedin && (
                <a href={social.linkedin} className="decoration-white text-blue-400" target="_blank">LinkedIn |</a>
              )}
              {social.github && (
                <a href={social.github} className="decoration-white text-blue-400" target="_blank"> GitHub</a>
              )}
              {social.portfolio && (
                <a href={social.portfolio} className="decoration-white text-blue-400" target="_blank"> | Portfolio</a>
              )}
            </p>
          )}
        </header>
              ...
              <div key={idx} className="mb-4">
                ...
                {proj.links && (
                  <div className="text-blue-600 underline space-x-4 mt-1">
                    {proj.links.linkedin && (
                      <a href={proj.links.linkedin} target="_blank" rel="noreferrer">LinkedIn</a>
                    )}
                    {proj.links.github && (
                      <a href={proj.links.github} target="_blank" rel="noreferrer">GitHub</a>
                    )}
                    {proj.links.portfolio && (
                      <a href={proj.links.portfolio} target="_blank" rel="noreferrer">Portfolio</a>
                    )}
                  </div>
                )}
              </div>
                  ...
                  <div key={i} className="mb-2">
                    ...
                    <ul className="list-disc list-outside pl-5">
                      {c.coursework.map((cw, j) => (
                        <li className="text-justify" key={j}>{cw}</li>
                      ))}
                    </ul>
                  </div>
                )
